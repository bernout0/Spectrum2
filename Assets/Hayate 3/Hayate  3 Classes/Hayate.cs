...
public class Hayate : MonoBehaviour
{
	...
	public AnimationCurve EmptyAnimationCurve()
	{
		...
		k1.time = 0;
		k1.value = -1f;
		k2.time = particleSystem.duration;
		k2.value = 1f;
		a.AddKey( k1 );
		a.AddKey( k2 );
	...
	public void EvaluateCurves()
	{
		...
		{
			Amplitude = new Vector3(AmplitudeCurveX.Evaluate(particleSystem.time),
			                        AmplitudeCurveY.Evaluate(particleSystem.time),
			                        AmplitudeCurveZ.Evaluate(particleSystem.time)
			                        );
		}

		if(useFrequencyCurve)
		{
			Frequency = new Vector3(FrequencyCurveX.Evaluate(particleSystem.time),
			                        FrequencyCurveY.Evaluate(particleSystem.time),
			                        FrequencyCurveZ.Evaluate(particleSystem.time)
			                        );
		}

		if(useOffsetCurve)
		{
			Offset = new Vector3(OffsetCurveX.Evaluate(particleSystem.time),
		                        OffsetCurveY.Evaluate(particleSystem.time),
		                        OffsetCurveZ.Evaluate(particleSystem.time)
		                        );
		}
		
		if(useOffsetSpeedCurve)
		{
			OffsetSpeed = new Vector3(OffsetSpeedCurveX.Evaluate(particleSystem.time),
		                        OffsetSpeedCurveY.Evaluate(particleSystem.time),
		                        OffsetSpeedCurveZ.Evaluate(particleSystem.time)
		                        );
		}

		if(useGlobalForceCurve)
		{
			GlobalForce = new Vector3(GlobalForceCurveX.Evaluate(particleSystem.time),
			                          GlobalForceCurveY.Evaluate(particleSystem.time),
			                          GlobalForceCurveZ.Evaluate(particleSystem.time)
			                        );
		}
		
		if(useParticleSpeedCurve)
		{
			particleSpeedToMesh = particleSpeedToMeshAnimation.Evaluate( particleSystem.time );
		}
	}
